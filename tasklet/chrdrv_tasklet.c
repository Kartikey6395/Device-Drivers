...
#define TIMEOUT 100
#define DEV dev_tasklet

struct tasklet_struct k_task;
static int i=1;

void k_tasklet_callback(unsigned long data)
{
    printk(KERN_INFO "Delay counter = %d && data = %d\n",i++,data++);
    
    tasklet_schedule(&k_task);
...
static int __init CharDevice_init(void)
{
    ...
    result = alloc_chrdev_region(&DevNum,11,1,"MyCharTasklet");    //dynamic register device region
    MAJOR = MAJOR(DevNum);
    MINOR = MINOR(DevNum);
    ...
    tasklet_init(&k_task,k_tasklet_callback,TIMEOUT);
...
ssize_t DEV_read(struct file *filp, char __user *Ubuff, size_t count, loff_t *offp)
{
    ...
    result = copy_to_user((int *)Ubuff,(int *)Kbuff,sizeof(Kbuff));
    if(result == 0)
    {
        printk("Data read successfully by user= %d",(int *)Ubuff);
