...
static int __init CharDevice_init(void)
{
    ...
    result = alloc_chrdev_region(&DevNum,11,1,"Dev_synch");
    MAJOR = MAJOR(DevNum);
    MINOR = MINOR(DevNum);
...
ssize_t DEV_read(struct file *filp, char __user *Ubuff, size_t count, loff_t *offp)
{
    ...
    flag = wait_for_completion_interruptible(&Wcomp);
    if(flag == 0)
   ...
   // flag = 1;
    ...
    if(result == 0)
    {
        printk("Reader 1 listen-->>%s",Ubuff);
        complete(&Rcomp);
...
ssize_t DEV_write(struct file *filp, const char __user *Ubuff, size_t count, loff_t *offp)
{
    ...
    if(result==0)
    {
        ...
        wait_for_completion_interruptible(&Rcomp);
    printk(KERN_INFO "writer says-->>-%s \n",Kbuff);
    up(&sem1);
        ret = count;
        return ret;
